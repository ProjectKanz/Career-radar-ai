...
const EVIDENCE_ID_PATTERN = /^(WRK|EDU|PRJ|CRT|SKL|OTH)-\d{3}-\d{3}$/;
...
export default function EvidenceBankPanel({ userId }: EvidenceBankPanelProps) {
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    if (!EVIDENCE_ID_PATTERN.test(normalizedEvidenceId)) {
      setFormError('Use the format WRK-001-001, EDU-001-001, PRJ-001-001, CRT-001-001, SKL-001-001, or OTH-001-001.');
      return;
    }
    ...
    if (duplicate) {
      setFormError(`Evidence ID ${normalizedEvidenceId} already exists. Use the next claim number or choose another ID.`);
      return;
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. WRK-001-001, PRJ-001-001"
                  value={evidenceId}
                  onChange={(e) => {
